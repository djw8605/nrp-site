...
:exclamation: Since we know that containers running in Nautilus aren't exposed to the broader internet, but it's critical to remember that they *are* exposed to other pods running in our namespace.  This is feature is beneficial when thinking about deploying complex software or services in Kubernetes.
...
:exclamation: It's an important skill to be able to iterate by modifying your existing YAML files. This is how most people develop new versions: they modify existing versions.
...
:question: Where might we look for a basic Ubuntu container that has `curl` preinstalled?
...
```
:exclamation:Notice that we've increased our resources and requests for memory and CPU.

:exclamation: When using a shared Kubernetes cluster (or any shared cluster for that matter), it is wise to balance your need for resources with their availability. Asking for too many resources will decrease the chances the cluster can provide them in a reasonable timeframe. Asking for too few resources will negatively impact your ability to compute.
...
:question: Try it a few times…by repeating the command (hint: the &uarr; on your keyboard will toggle through previous commands). Which Web server is serving you? Does it matter? How could you imagine using a `service` like this to balance the load across multiple pods or deployments?


:exclamation:Note that you can also use the local DNS name for this (from pod1)

`curl http://test-svc.<namespace>.svc.cluster.local`

:question:Do you remember what namespace you are using?
...
:exclamation: Please make sure you did not leave any running pods, deployments, ingresses or services behind.
