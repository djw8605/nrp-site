...
2. [Install kubelogin](https://github.com/int128/kubelogin?tab=readme-ov-file#setup) plugin
    ...
    Example for Linux or OSX (`curl`, `jq`, and `unzip` should be installed):

    ```bash
    # OS_ARCHITECTURE should be in the format `amd64` or `arm64`
    # For Debian/Ubuntu - change as adequate for RHEL or OSX
    OS_ARCHITECTURE="$(dpkg --print-architecture)"

    # Install kubelogin
    KUBELOGIN_VERSION="$(curl -fsSL "https://api.github.com/repos/int128/kubelogin/releases/latest" | jq -r '.tag_name')"
    curl -o kubelogin.zip -fSL "https://github.com/int128/kubelogin/releases/download/${KUBELOGIN_VERSION}/kubelogin_linux_${OS_ARCHITECTURE}.zip"
    unzip kubelogin.zip kubelogin
    chmod +x ./kubelogin
    # Replace `/usr/local/bin` to `~/.local/bin` and set `export PATH="~/.local/bin:${PATH}"` for local user
    sudo mv ./kubelogin /usr/local/bin/kubectl-oidc_login
    sudo chown root: /usr/local/bin/kubectl-oidc_login
    rm -f kubelogin.zip kubelogin
    ```
