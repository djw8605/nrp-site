...
3. **Cross-platform kubelogin fixes (WSL / Linux / Windows / other non-Mac)**

    If you run into authentication issues with `kubelogin`:

    - **Keyring errors** (e.g., `/run/user/1000/bus` not found):
      ```bash
      --token-cache-storage=disk
      ```
      (stores tokens on disk instead of Linux keyring)
