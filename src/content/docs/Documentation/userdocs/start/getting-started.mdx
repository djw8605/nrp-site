...
description: Log in through CILogon, get added to a namespace, and install the tools you need to reach the Nautilus cluster.
---
...
If you are a new user and want to access the Nautilus cluster, follow the steps below.

:::note[The namespaces manager has moved]
The namespaces management page is now at [https://nrp.ai/namespaces](https://nrp.ai/namespaces). If you can't find the namespace in the old portal, try the new one.
:::

<Steps>

1. Point your browser to the [NRP portal](https://nrp.ai).

2. On the portal page, click on the **`Log In`** button at the top right corner.

3. You will be redirected to **`CILogon`** — the federated login service shared by research institutions — where you have to **Select an Identity Provider**.
...
7. If you are a **faculty member, university staff member, or postdoc** starting a new project and need your own [namespace](/documentation/userdocs/start/using-nautilus/#namespace) — either for yourself or your research group — request one by filling out the [namespace request form](https://get-started.nrp-nautilus.io). Log in at [nrp.ai](https://nrp.ai) first so the request is tied to your account; students should have their advisor submit the form. The form asks for the desired name for your group (usually the department name) — use lowercase letters, numbers, and dashes only, and choose clear, descriptive names (avoid generic ones like `kubernetes-ai`, `testing-group`, `llm-access`). To see the existing names for reference, visit this [visualization tree](https://nrp.ai/viz/namespaces/). If you have questions, ask in [Nautilus Support](/contact).
As an **`admin`**, you will have the ability to: 
    ...
    The form asks for your current affiliation; it helps to think about where you see yourself fitting in this structure. You can simply own a single namespace under your university branch for small experiments
...
2. **Make sure you read the cluster policies before starting to use it.**
    :::caution[Read these before you run anything]
...
3. **Make sure you're on Nautilus Support. All Nautilus and NRP users are expected to join for user support and real-time updates.**
    :::note[Join the Nautilus Support chat]
    - If you do not yet have access to Nautilus Support, please register right away following the instructions at <a href="https://nrp.ai/contact/" target="_blank">**this link.**</a>
    :::
</Steps>
...
## Cluster access via kubectl
    ...
    - Please do not skip any steps. <b>All steps below are necessary to be able to connect to Nautilus.</b>
    - Common Kubelogin connectivity issues are addressed in Step 3. Please view carefully if you run into any issues.
    :::

<Steps>

1. [Install][1] the Kubernetes command-line tool, `kubectl` from the [official instructions][1] of each OS.

2. [Install kubelogin](https://github.com/int128/kubelogin?tab=readme-ov-file#setup) plugin

    :::caution[Install kubelogin first]
    You **must** install the `kubelogin` plugin, or your kubeconfig file **will not work**.
    :::

    <LinkButton href="https://github.com/int128/kubelogin?tab=readme-ov-file#setup">
    Download kubelogin
    </LinkButton>
...
3. **Cross-platform kubelogin fixes (WSL / Linux / Windows / macOS)**
...
```bash title="Invalidate access token"
kubectl oidc-login clean
```

After that any `kubectl` command will trigger getting the new token. (F.e. `kubectl get nodes`)


:::danger[Read this before you run anything on the cluster]
- Containers are stateless. 
    - **All your data will be gone forever when the container restarts**, unless you store it in a persistent volume.
- Container restart is normal in the Kubernetes (k8s) cluster. Expect it.
...
Install both the [Kubernetes](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.vscode-kubernetes-tools) and [Remote Development](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.vscode-remote-extensionpack) extensions.
