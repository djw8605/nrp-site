...
:::note
The above usage limits implies:
- CPU `limit` must be less than 200% of the `request`.
    - For example: if you request 4 CPU cores, your limit must be 8 or less.
- Memory (RAM) `limit` must be less than 150% of the `request`.
- **We strongly recommend keeping resource `limits` within 20% of the `requests` for optimal performance.**.
:::
