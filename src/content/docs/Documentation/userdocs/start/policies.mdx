...
5. If a pod exceeds the CPU limit, it puts pressure on the node. It can affect the whole node and other user's work.
...
1. A single user can't submit more than 4 pods violating the following requirements (1 is 100%):
    ...
    - **CPU:** 0.2 - 2    
        &#8658; CPU usage should be >20% and < 200% of the `request`.   
    - **Memory:** 0.2 - 1.5    
...
:::note
- CPU `limit` must be < 200% of the `request`.
    - Example: if you request 4 cpu cores, your limit must not be more than 8 cpu cores.
...
3. If you need to run [some permanent services](/documentation/userdocs/running/ingress/) beyond two weeks, you can contact admins in [Matrix](/contact) and ask for an exception. 
...
While you are welcome to use as many resources as needed, using those inefficiently is causing problems to others. The CPU load of the system consists of user load, system load, and several others. If you run `top`, you'll see something like:
