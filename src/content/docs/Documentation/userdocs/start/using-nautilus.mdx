...
> **Important:** On Nautilus, pods that write more than **10Gi** of ephemeral scratch data per container (for example, to an `emptyDir`) can be evicted. If you attach an `emptyDir` scratch volume and expect to write more than 10Gi, set `resources.requests.ephemeral-storage` (and optionally `resources.limits.ephemeral-storage`) in your pod spec to the size you need. See the [Storage tutorial](/documentation/userdocs/tutorial/storage) and [Local Scratch](/documentation/userdocs/storage/local) pages for complete YAML examples that show both `emptyDir` and `ephemeral-storage` configured together.
