...
description: guide to use the cluster using kubernetes, jupyterhub and coder
---
import { LinkButton } from '@astrojs/starlight/components';

Nautilus is a Kubernetes cluster distributed all over the US and there are a few sites in Asia and Europe.
 Currently there are over 75 sites as part of the NRP. See all the sites on the <a href="https://dash.nrp-nautilus.io/" target="_blank">NRP dashboard</a>.
...
Check the <a href="https://nrp.ai/viz/resources"  target="_blank">**Resources**</a> page on the portal to see the detailed summary of each node.
...
There are three main ways to use the computing resources on the Nautilus cluster. 
...
### Coder Platform
[Coder](/documentation/userdocs/coder/coder) provides an easy-to-use, JupyterHub-like experience, and you can use the Nautilus cluster without Kubernetes knowledge. [Coder](https://coder.com/) also runs on the web browser. You can run your code without worrying about the underlying infrastructure. You can access the NRP-hosted Coder platform by visiting the [Coder](https://coder.nrp-nautilus.io) link and logging in with your institutional credentials using **OpenID Connect** (once the cluster admins approve your account).
...
- Cluster admins need to approve your account. Ask for access in [Nautilus Support](/contact).
- Your institution needs to be part of **`CILogon`**.
...
and it is possible to have access to multiple namespaces.
 Each namespace has at least one namespace admin (usually researchers, engineers, or faculties).
 If you are new to the cluster, please follow the instructions on the [Getting Started](/documentation/userdocs/start/getting-started#get-access-and-log-in) page.

:::tip[Who can be a Namespace Admin]
- A **faculty member, researcher, or postdoc**. Request a namespace with the [namespace request form](https://get-started.nrp-nautilus.io) — it asks for your name, institution, and preferred namespace name. Questions can go to [Nautilus Support](/contact).
- A student (preferably a Ph.D. or Masters student) can be a namespace admin if their supervisor submits the [form](https://get-started.nrp-nautilus.io) on their behalf.
