...
[pi.dev](https://pi.dev) — press `Ctrl-L` to switch models and search for `NRP` (or type `/model modelname`).
...
[charmbracelet/crush](https://github.com/charmbracelet/crush) — see also Andrea Zonca's writeup: [Configure NRP LLM with OpenCode and Crush](https://www.zonca.dev/posts/2026-01-29-configure-nrp-llm-opencode-crush).
...
[anthropics/claude-code](https://github.com/anthropics/claude-code) talks to NRP through the Anthropic-compatible endpoint at `/anthropic`. Set the variables below in your shell or `~/.claude/.env`:
