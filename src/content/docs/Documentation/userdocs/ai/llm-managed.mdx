...
Go to _Settings_ → _Model Provider_ → press the _Add_ button (_Provider Name_ to `NRP` or anything else you want, and _Provider Type_ to `OpenAI`) → add _API Key_ and _API Host_ (`https://ellm.nrp-nautilus.io/v1`) → press _Fetch model list_ → press the _Add models to the list_ button at the right of the search box to add all models.
...
Generate the config for it in the [LLM token generation page](/llmtoken). Copy the generated config to clipboard - it will already have your personal token. **Please leave `Max Output Tokens` empty (unless you know what you are doing; only fill in `Context Window` and read the _API Access to LLMs via Envoy gateway_ section).**
