---
title: NRP-Managed LLMs
description: Hosted open-weights LLMs on the National Research Platform
---
import { LinkCard, CardGrid } from '@astrojs/starlight/components';

The NRP hosts a rotating catalog of open-weights LLMs, accessible either through hosted **chat interfaces** in the browser or programmatically via an **OpenAI-compatible API**. Models are added and retired as the open-weights frontier moves.

<CardGrid>
  <LinkCard
    title="Chat with NRP LLMs →"
    href="https://nrp-openwebui.nrp-nautilus.io"
    description="Open WebUI in your browser — no setup required" />
  <LinkCard
    title="Get an API token →"
    href="/llmtoken"
    description="Programmatic access via OpenAI-compatible endpoint" />
</CardGrid>

## Pick a model

Every active model is compared side-by-side in the feature matrix, and each has a card with strengths, trade-offs, and recommended uses. A few quick recommendations:

- **Frontier reasoning, longest context, multimodal** — `qwen3`
- **Agentic coding** — `kimi`, `glm-4.7`, or `minimax-m2`
- **Multimodal with audio** (ASR, speech-to-text) — `gemma-small`
- **Reproducible research / general-purpose LTS** — `gpt-oss`
- **Embeddings for vector search** — `qwen3-embedding`

<CardGrid>
  <LinkCard
    title="Available models & feature matrix"
    href="/documentation/userdocs/ai/llm-managed/models"
    description="Compare every model side-by-side; jump to any model card" />
  <LinkCard
    title="Model lifecycle & changelog"
    href="/documentation/userdocs/ai/llm-managed/lifecycle"
    description="What's been added, deprecated, or removed — and why" />
</CardGrid>

## Use the LLMs

For browser chat, the **NRP Open WebUI** is the most full-featured option. We also host LibreChat, and the same OpenAI-compatible API works with desktop apps like Cherry Studio and Chatbox.

For programmatic access, the OpenAI-compatible endpoint at `https://ellm.nrp-nautilus.io/v1` works with the OpenAI Python client, `curl`, or any compatible library. The same page also covers `cache_salt` — recommended for any tenant where prompts shouldn't be cached across users.

For coding CLIs, ready-to-paste configs are provided for **Claude Code**, **OpenCode**, **Crush**, **Kimi CLI**, **pi**, and **Copilot CLI**.

<CardGrid>
  <LinkCard
    title="Chat interfaces"
    href="/documentation/userdocs/ai/llm-managed/chat-interfaces"
    description="Open WebUI, LibreChat, Cherry Studio, and Chatbox" />
  <LinkCard
    title="API access"
    href="/documentation/userdocs/ai/llm-managed/api-access"
    description="Endpoint, examples, and cache_salt isolation" />
  <LinkCard
    title="Client configurations"
    href="/documentation/userdocs/ai/llm-managed/client-configs"
    description="Configs for the major coding CLIs" />
</CardGrid>

## Operate responsibly

The NRP applies a **Fair Use Policy** with per-model concurrency limits — please review it before sending high-volume traffic. SDSC and Internet2 affiliates get higher limits. For real-time capacity and health, the **LLM Status** page surfaces live Prometheus / vLLM metrics, with one card per Kubernetes container.

<CardGrid>
  <LinkCard
    title="Fair use policy"
    href="/documentation/userdocs/ai/llm-managed/fair-use"
    description="Per-model concurrency limits and retry guidance" />
  <LinkCard
    title="Live LLM status"
    href="/llm-status"
    description="Live deployment health from Prometheus / vLLM" />
</CardGrid>

## Deploy your own model

If the managed catalog doesn't fit your need, the NRP supports running your own LLM via vLLM or SGLang. See [Managing AI Models](/documentation/admindocs/ai/managing_models/) for the deployment path.
