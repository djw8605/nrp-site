---
title: Ceph S3
description: "Create buckets on the platform Ceph storage and read or write them from any S3 client."
---
...
| --------------------- | ------------------------------------------- | ---------------------------------- |
| _West pool (default)_ | http://rook-ceph-rgw-nautiluss3.rook        | https://s3-west.nrp-nautilus.io    |
| _Central pool_        | http://rook-ceph-rgw-centrals3.rook-central | https://s3-central.nrp-nautilus.io |
| _East pool_           | http://rook-ceph-rgw-easts3.rook-east       | https://s3-east.nrp-nautilus.io    |
| _HaoSu pool_          | http://rook-ceph-rgw-haosu.rook-haosu       | https://s3-haosu.nrp-nautilus.io   |
| _Tide pool_           | http://rook-ceph-rgw-tide.rook-tide         | https://s3-tide.nrp-nautilus.io    |
| _HA pool_             | https://s3-ha.nrp-nautilus.io               | https://s3-ha.nrp-nautilus.io      |
...
The HA pool replicates every bucket to the west, central and east clusters. Use `https://s3-ha.nrp-nautilus.io` from inside the cluster as well: it is the only endpoint that fails over (within about 15 seconds). The per-zone URLs `http://rook-ceph-rgw-ha.rook:7480`, `http://rook-ceph-rgw-ha.rook-central:7480` and `http://rook-ceph-rgw-ha.rook-east:7480` serve one zone only and stop with it. Objects written while a zone was down show up there shortly after it returns. It is reserved for mission-critical services; the portal does not issue its credentials, so ask the admins if your service needs it.
...
_Storage_: Amazon S3 Compliant Storage Providers

_S3 provider_: Ceph Object Storage

_AWS Access Key ID, AWS Secret Access Key_: [Get S3 keys](#access)

_Endpoint_: [check the regions section](#s3-regions-settings)
...
We recommend to use **[awscli-plugin-endpoint][10]** to write endpoint url in **.aws/config**, instead of typing endpoint in the CLI repeatedly. Install the plugin with:

```
pip install awscli-plugin-endpoint
```

There are a few steps on the **[awscli-plugin-endpoint][10]** README.md to install this plugin. If you do not wish to add this plugin, add `--endpoint-url https://s3-west.nrp-nautilus.io` to all commands below.
...
4. Upload a file and make it publicly accessible:
   ...
   [https://s3-west.nrp-nautilus.io/my-bucket/hello.txt](https://s3-west.nrp-nautilus.io/my-bucket/hello.txt)
...
S3 Region Settings
First, determine which S3 pool and region you are using. Your endpoint should follow this format based on your region:
...
See this [TFRecord][2] presentation for details.
