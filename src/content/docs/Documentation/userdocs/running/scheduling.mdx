...
:::caution[Job specs are visible to every user]
  Job specs (the YAML you submit) are visible to every user of the cluster, and there is currently no way to restrict that per namespace. Do not put secrets, credentials, or unpublished data in a job spec.
:::
