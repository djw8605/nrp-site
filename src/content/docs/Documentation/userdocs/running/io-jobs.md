...
- Store your dataset on the PRP/[S3](/documentation/userdocs/storage/ceph-s3/) interface. The S3 interface is scalable and universally accessible, so it make a great place to store large datasets which can be simultaneously accessed either from inside the cluster or externally.
