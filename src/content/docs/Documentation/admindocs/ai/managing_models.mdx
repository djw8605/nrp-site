...
Note that `--enable-request-id-headers --otlp-traces-endpoint <endpoint>`, `OTEL_EXPORTER_OTLP_TRACES_INSECURE` to `true`, and `OTEL_SERVICE_NAME` to the service name are optional and used typically only for the NRP-hosted models. For SGLang, use `--log-requests --log-requests-level 1 --enable-metrics --enable-trace --otlp-traces-endpoint <endpoint>` with `OTEL_EXPORTER_OTLP_TRACES_INSECURE` and `OTEL_SERVICE_NAME`. Moreover, make sure to add `--api-key "${VLLM_API_KEY}"` for SGLang.
    ...
    spec:
      ...
      - args:
        ...
        - name: VLLM_ENGINE_READY_TIMEOUT_S
          value: "86400"
        # Use in case CUDA memory errors occur; may solve the problem over lowering --gpu-memory-utilization
...
Two negative priority classes exist: `nice` (`value: -10`) and `opportunistic` (`value: -2000000000`, preempted first). Current scavenging replicas: `qwen3-27b-h200` and `gemma-bw-vllm-inference` run as `opportunistic`; `glm-5-vllm-inference-h100`, on a partner-owned node, runs as `nice`.

- Pattern: scavenging instances are additional replicas of the normal managed deployment (same served model id, same pool label or service group), not a separate model endpoint
...
[nrp.ai/llm-status](https://nrp.ai/llm-status/) is driven by Prometheus/Thanos, not by the
Kubernetes API. A model can be `Running`, `Ready`, and serving traffic through the gateway while
being completely absent from the status page. Three rules decide whether a workload shows up.

### 1. The workload needs a Service labelled `component: llm`

`ServiceMonitor/llm-monitor` discovers scrape targets through **Services**, so a StatefulSet with no
Service is never scraped at all. This is easy to miss because routing does not depend on it:
`InferencePool` selects pods by **label**, so a model with no Service routes perfectly and reports no
metrics anywhere.

```bash
# every vLLM workload must return a Service here
kubectl get svc -n nrp-llm -l component=llm
```

### 2. The container name must be unique, and should match the Service name

The status page keys models on `namespace/container`, and it joins two independent metric families:

```promql
sum(vllm:num_requests_running) by (namespace, container, model_name)   # from vLLM
count(DCGM_FI_DEV_GPU_UTIL)    by (namespace, container, modelName)    # from DCGM
```

Two consequences:

- **Two workloads sharing a container name collapse into one row.** Naming several containers
  `vllm` puts every one of them on the single key `namespace/vllm`.
- **DCGM reports the *real* container name and cannot be relabelled.** Rewriting `container` in the
  `ServiceMonitor` fixes the vLLM half and silently breaks the DCGM half — the model keeps its row but
  loses `gpu_count` and `gpu_utilization`, and its GPUs get attributed to a phantom row. Rename the
  container in the manifest instead.

### 3. Metrics only exist after the model serves its first request

vLLM does not emit `vllm:num_requests_running`, `vllm:num_requests_waiting`, or
`vllm:kv_cache_usage_perc` until a request has been processed. Since the status page *discovers*
models with `sum(vllm:num_requests_running)`, **a freshly deployed or freshly restarted model is
invisible until traffic reaches it** — even with a healthy, `up` scrape target. Counters such as
`vllm:generation_tokens_total` are present from boot, so a target that exposes those but no gauges is
simply cold, not broken.

After deploying or restarting a model, send it one request:

```bash
curl -s https://ellm.nrp-nautilus.io/v1/chat/completions \
  -H "Authorization: Bearer $NRP_LLM_TOKEN" -H 'Content-Type: application/json' \
...
```bash
# is it scraped, and is the target healthy?
curl -s 'https://prometheus.nrp-nautilus.io/api/v1/targets?state=active' \
  | jq -r '.data.activeTargets[] | select(.scrapePool | test("llm-monitor")) | "\(.labels.pod) \(.health) \(.lastError)"'

# do the two metric families agree on the container name?
curl -s -G https://thanos.nrp-nautilus.io/api/v1/query \
  --data-urlencode 'query=sum(vllm:num_requests_running) by (namespace, container)'
curl -s -G https://thanos.nrp-nautilus.io/api/v1/query \
  --data-urlencode 'query=count(DCGM_FI_DEV_GPU_UTIL{namespace="nrp-llm"}) by (namespace, container)'
```

A container appearing in the DCGM output but not the vLLM output is either cold (rule 3) or not
scraped (rule 1). A container appearing in neither, while its pod is `Running`, has no Service.
...
:::caution
When changing the NRP-provided models, also update the [model catalog](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/tree/main/src/content/models) (`src/content/models/`, one YAML per model), the [Chatbox config template](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/blob/main/src/components/vue/LLMToken.vue), the [client configurations](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/blob/main/src/content/docs/Documentation/userdocs/ai/llm-managed/client-configs.mdx) and the [changelog](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/blob/main/src/content/docs/Documentation/userdocs/ai/llm-managed/lifecycle.mdx).
:::
...
`backendRefs` point at an [AIServiceBackend](#aiservicebackend) for a single endpoint, or at an `InferencePool` (`group: inference.networking.k8s.io`) for a pooled model, whose endpoint picker in `inference-pool/inferencepool-<model>.yaml` routes on queue depth, KV-cache usage and prefix-cache locality. `modelNameOverride` must be a name every pod in the pool advertises with `--served-model-name`, or that pod answers 404.
...
spec:
  ...
  rules:
    # A pooled model: the EPP behind InferencePool/qwen3 picks the pod
    - matches:
        - headers:
            - type: Exact
              name: x-ai-eg-model
      ...
      backendRefs:
        - group: inference.networking.k8s.io
          kind: InferencePool
          name: qwen3
          modelNameOverride: Qwen/Qwen3.8-Flash-Next-FP8
      timeouts:
        request: 1200s
      modelsOwnedBy: "NRP"
    # A single endpoint: AIServiceBackend -> Backend -> Service
    - matches:
        - headers:
            - type: Exact
              name: x-ai-eg-model
      ...
      backendRefs:
        - name: envoy-ai-gateway-nrp-qwen3-embedding
          modelNameOverride: Qwen/Qwen3-VL-Embedding-8B
      timeouts:
        request: 1200s
      modelsOwnedBy: "NRP"
    - matches:
...
spec:
  targetRefs:
    ...
    - group: gateway.networking.k8s.io
      kind: HTTPRoute
      name: envoy-ai-gateway-nrp-deepseek-v4-flash
    - group: gateway.networking.k8s.io
      kind: HTTPRoute
      name: envoy-ai-gateway-nrp-gemma-4-12b
  retry:
    numRetries: 2  # Total retry attempts across all backendRefs; 404 is not retried
    perRetry:
      backOff:
        baseInterval: 100ms
        maxInterval: 10s
  ...
  # 200,000 output tokens per minute per API token and model (input is not counted)
  rateLimit:
    type: Global
    global:
      rules:
...
metadata:
  name: envoy-ai-gateway-nrp-kimi
  namespace: nrp-llm
spec:
  schema:
    name: OpenAI
...
spec:
  ...
  endpoints:
    - fqdn:
        hostname: kimi-vllm-inference.nrp-llm.svc.cluster.local
        port: 5000
```
...
It's easier if you reuse one of existing keys and simply add your backend to the list in one of existing BackendSecurityPolicies. The `BackendSecurityPolicy` targets an existing `AIServiceBackend` or `InferencePool`; `envoy-ai-gateway-nrp-apikey` carries the `VLLM_API_KEY` every NRP vLLM pod starts with.
...
spec:
  ...
  targetRefs:
    ...
    - name: envoy-ai-gateway-nrp-minimax
      kind: AIServiceBackend
      group: aigateway.envoyproxy.io
    - name: envoy-ai-gateway-nrp-qwen3-embedding
      kind: AIServiceBackend
...
Finally, update the site: the [model catalog](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/tree/main/src/content/models), the [Chatbox Config Template](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/blob/main/src/components/vue/LLMToken.vue), the [client configurations](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/blob/main/src/content/docs/Documentation/userdocs/ai/llm-managed/client-configs.mdx) and the [changelog](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/blob/main/src/content/docs/Documentation/userdocs/ai/llm-managed/lifecycle.mdx).
