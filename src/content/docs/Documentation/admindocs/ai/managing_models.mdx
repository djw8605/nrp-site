...
Normally, in models without sliding window attention (SWA) capabilities, the KV cache size needs to be above the context size of the model, when the model does not use sliding window attention.
...
But when models use **sliding window attention (SWA)**, it is possible that lower KV cache size than the context size of the model accommodates full context.
...
Based on the above information, edit the below configuration files to deploy the model.
