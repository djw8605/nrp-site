...
:::caution
If you are a namespace administrator reading this manual for your own namespace, use a relatively high `ingress.kubernetes.io/timeout-server` annotation on your **Ingress** if you experience any timeouts.
:::
  ...
  template:
    ...
    spec:
      ...
      - args:
        ...
        envFrom:
        - configMapRef:
            # Default configMap for nrp-llm, refer to other models in the namespace for sdsc-llm
        ...
        resources:
          limits:
            # Minimum Threads = 2 * (Number of GPUs + API Server Count (Minimum 1) + Data Parallelism Count (Minimum 1) + (1 if Data Parallelism Count > 1 else 0)); Set maximum possible if offloading to CPU RAM
            cpu: "32"
    ...
    spec:
      affinity:
        nodeAffinity:
          requiredDuringSchedulingIgnoredDuringExecution:
            nodeSelectorTerms:
            - matchExpressions:
              # Newer than Pascal GPUs (GTX 1000, https://en.wikipedia.org/wiki/CUDA#GPUs_supported)
      ...
      - args:
        ...
        - configMapRef:
            # Default configMap for nrp-llm, refer to other models in the namespace for sdsc-llm
            name: qwen-vllm-inference-config
        # Use nightly container when recent commit fixes model
...
In the above case, the `Backend` for Envoy AI Gateway should be configured as `hostname: minimax-vllm-inference.nrp-llm.svc.cluster.local` and `port: 5000`.
...
1. Read individual instructions for each model (both from Hugging Face and vLLM/SGLang) carefully, and also check deployment configurations from other models. The recommended number of CPU cores to request is `Minimum Threads = 2 * (Number of GPUs + API Server Count (Minimum 1) + Data Parallelism Count (Minimum 1) + (1 if Data Parallelism Count > 1 else 0))`; read below for API Server Count (`--api-server-count`) and Data Parallelism Count (`--data-parallel-size`). Set CPU thread counts to the maximum possible if offloading to CPU RAM. The recommended RAM size is `request: [slightly over half of total loaded model size]`, `limit: [slightly over total loaded model size]`.
...
The core equation for CPU thread consumption is: `Minimum Threads = 2 * (Number of GPUs + API Server Count (Minimum 1) + Data Parallelism Count (Minimum 1) + (1 if Data Parallelism Count > 1 else 0))`.

**Set maximum threads possible if offloading weights or KV cache to CPU RAM.**

1. Increase internal API server count (`--api-server-count`) to a value larger than `1` (perhaps `4` for smaller models or `8` for larger models); this allows multi-core processing for tokenization and input processing.
...
2. (**Decreases available KV cache by potentially a large amount, so only when VRAM is surplus in large-context models**) Increase `--max-num-batched-tokens` to around `16384` but below `--max-model-len`. Default is currently `8192`.

### CPU Offloading

Key environment variables and arguments related to CPU offloading:
- `--offload-backend <prefetch/uva>`, `--cpu-offload-gb <size_in_GB>`, `--offload-group-size`, `--cpu-offload-params`, etc. (refer to `OffloadConfig` in https://docs.vllm.ai/en/latest/configuration/engine_args/)
- `VLLM_WEIGHT_OFFLOADING_DISABLE_PIN_MEMORY=1` (mandatory for all offloading usage cases in the meantime), `VLLM_WEIGHT_OFFLOADING_DISABLE_UVA=1` when `--offload-backend prefetch` is set (refer to https://github.com/vllm-project/vllm/pull/32993)
- In multimodal models, multimodal CPU RAM cache with `--mm-processor-cache-gb <size_in_GB>` (frequently paired with `--mm-processor-cache-type shm` in more than 1 GPU)
...
Example for [MiniMaxAI/MiniMax-M2.5](https://huggingface.co/MiniMaxAI/MiniMax-M2.5): `--max-model-len 262144 --hf-overrides '{"max_position_embeddings": 262144, "rope_scaling": {"type": "dynamic", "factor": 1.5, "original_max_position_embeddings": 196608}}'`
...
- [hf-mem](https://pypi.org/project/hf-mem/) is useful, but the KV cache estimate may be inaccurate, **especially due to models that can take more context with sliding window attention (SWA)**. Assume that the KV cache is bfloat16 to be conservative, but **do not give up as long as the weights fit in the total VRAM** (or RAM when CPU offload is used). Something like `uvx hf-mem --model-id MiniMaxAI/MiniMax-M2.5 --experimental --max-model-len 196608 --batch-size 1 --kv-cache-dtype bfloat16` is an example that can be used.
