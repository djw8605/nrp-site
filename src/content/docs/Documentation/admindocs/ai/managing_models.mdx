...
**Make sure to delete the `rules:` and update the `AIGatewayRoute` with `kubectl apply -f <file>` if a model is removed. If all models under `rules:` were deleted, make sure to delete the `AIGatewayRoute` manually.**
...
**Make sure to delete the `AIServiceBackend` manually if a model is removed.**
...
**Make sure to delete the `Backend` manually if a model is removed.**
...
**Make sure to delete the `targetRefs:` section and update the `BackendSecurityPolicy` with `kubectl apply -f <file>`. Make sure to delete the `BackendSecurityPolicy` manually if all models under `targetRefs:` are removed.**
