...
1. Read individual instructions for each model (both from Hugging Face and vLLM/SGLang) carefully, and also check deployment configurations from other models. The recommended number of CPU cores to request is `Minimum Threads = 2 * (Number of GPUs + API Server Count (Minimum 1) + Data Parallelism Count (Minimum 1) + (1 if Data Parallelism Count > 1 else 0)); Set maximum possible if offloading to CPU RAM` (read below for API Server and Data Parallelism counts), and the recommended RAM size is `request: [slightly over half of total loaded model size]`, `limit: [slightly over total loaded model size]`.
...
- `VLLM_WEIGHT_OFFLOADING_DISABLE_PIN_MEMORY=1`, `VLLM_WEIGHT_OFFLOADING_DISABLE_UVA=1` (refer to https://github.com/vllm-project/vllm/pull/32993)
- `VLLM_CPU_KVCACHE_SPACE` (in GB), `--mm-processor-cache-gb` (in GB for multimodal models, frequently paired with `--mm-processor-cache-type shm` in more than 1 GPU)
