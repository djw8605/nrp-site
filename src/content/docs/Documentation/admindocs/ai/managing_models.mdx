...
[hf-mem](https://pypi.org/project/hf-mem/) is a useful way to determine the total size of weights loaded to the system, and an **incomplete** estimate of KV cache requirements.

Something like `uvx hf-mem --model-id MiniMaxAI/MiniMax-M2.5 --experimental --batch-size 1 --kv-cache-dtype bfloat16` is an example that can be used.
...
**IMPORTANT:** The KV cache estimate is likely inaccurate in many cases, **especially due to models that can take more context by using sliding window attention (SWA)**. Read the Hugging Face description of the model to see whether sliding window attention (SWA).

**Example:** Normally, the KV cache size needs to be above the context size of the model, when the model does not use sliding window attention.
...
**Example:** But when using **sliding window attention (SWA)**, it is possible that lower KV cache size than the context size of the model accommodates full context.

```
(EngineCore pid=160) INFO 00-00 00:00:00 [kv_cache_utils.py:1316] GPU KV cache size: 145,040 tokens
(EngineCore pid=160) INFO 00-00 00:00:00 [kv_cache_utils.py:1321] Maximum concurrency for 262,144 tokens per request: 2.17x
```

Because of the above reasons, assume that the KV cache is `bfloat16` to be conservative, but **do not give up fitting the model into available VRAM due to the reported KV cache in `hf-mem`, as long as the weights fit in the total VRAM** (or CPU RAM when CPU offload is used). Moreover, vLLM/SGLang may use `fp8` KV cache automatically when the training of the model itself was performed quantized (like DeepSeek), and `hf-mem` cannot automatically detect this situation.
  ...
  template:
    ...
    spec:
      ...
      - args:
        ...
        # Use latest release, and use nightly container if recent commit fixes model or tool calling
...
:::caution
**This section is mainly intended for administrators of the cluster-wide managed LLM models.** If you are a namespace administrator reading this manual for your own namespace, use a relatively high `ingress.kubernetes.io/timeout-server` annotation on your **Ingress** resource, if you experience any timeouts.
:::
