...
A BackendTrafficPolicy is required for failovers in multiple `backendRefs:` endpoints. Add an `HTTPRoute`, which is automatically generated with the same name as the `AIGatewayRoute`, to the existing `BackendTrafficPolicy`.
