...
NOTE: While context parallelism is only sometimes a concern since quite a few large models have over 16 to 128 `num_key_value_heads`, this should be considers when using multi-head latent attention (MLA), where the KV cache is compressed into a lower-dimensional latent space, and the number of KV heads decreases to 1 (mainly in DeepSeek-V3/R1 series and derived models such as Kimi-K2).
