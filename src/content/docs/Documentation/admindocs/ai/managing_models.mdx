...
:::caution
You are not supposed to use any tolerations to taints, unless you have been approved by administrators for one or are an administrator yourself. Please refer to the [Special Use](/documentation/userdocs/running/special/) on this matter.
:::
          ...
          requiredDuringSchedulingIgnoredDuringExecution:
            ...
            - matchExpressions:
              ...
              - key: nvidia.com/gpu.memory
                ...
                values:
                - "80000"
              # At least GTX 2000 Series Turing GPUs (CUDA Capability 6.x) for CUDA 12 (use the `cu129-` image prefix; `v0.19.0` is the last version supporting GTX 1000 Series Pascal and Volta), Newer than Volta GPUs (CUDA Capability 7.0) for CUDA 13; https://en.wikipedia.org/wiki/CUDA#GPUs_supported
...
spec:
  ...
  selector:
    app: kimi-bw-vllm-inference
    statefulset.kubernetes.io/pod-name: kimi-bw-vllm-inference-0
  ports:
  - name: http
    port: 5000
    targetPort: 5000
...
spec:
  ...
  persistentVolumeClaimRetentionPolicy:
    whenDeleted: Retain
    whenScaled: Retain
  # Required for deploying all instances at the same time
  ...
  template:
    ...
    spec:
      affinity:
        ...
        # 2. Prefer pods to be scheduled close together (on the same node)
        podAffinity:
            ...
            podAffinityTerm:
              labelSelector:
                ...
                - key: app
                  operator: In
                  values:
                  - kimi-bw-vllm-inference
              topologyKey: kubernetes.io/hostname
      containers:
      - args:
        - |
          # Extract the StatefulSet ordinal index from the hostname to use as the node rank
          RANK=${HOSTNAME##*-}
          
          # Address of the head node (Pod 0). 
          MASTER_ADDR="kimi-bw-vllm-inference-0.kimi-bw-vllm-master.nrp-llm.svc.cluster.local"
          
          HEADLESS_FLAG=""
          API_SERVER_COUNT_FLAG=""
          
          if [ "$RANK" != "0" ]; then
            HEADLESS_FLAG="--headless"
          else
            API_SERVER_COUNT_FLAG="--api-server-count 8"
          fi
        ...
        - configMapRef:
            name: qwen-vllm-inference-config
        image: vllm/vllm-openai:nightly-7b1bc0a3eb01a6bc2650eda9970049f7825240d7
        imagePullPolicy: Always
        livenessProbe:
          failureThreshold: 8
          httpGet:
...
:::caution
When changing the NRP-provided models, also change the [Chatbox config template](https://gitlab.nrp-nautilus.io/prp/nrp-site/-/blob/main/src/components/vue/LLMToken.vue).
:::
