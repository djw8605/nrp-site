...
But when models use **sliding window attention (SWA)**, a lower KV cache size than the context size of the model can accommodate the full context size.
...
**Important:** It is entirely possible that when a high pipeline parallelism count is used to have vLLM wait a long time after `Application startup complete.`, then the requests come in; this is the LLM engine initializing the pipelines. Moreover, there may be an error `raise TimeoutError(f"RPC call to {method} timed out.")`.

**So when using pipeline parallelism**, add the below environment variables to eliminate the timeout error and wait until the responses come in (this may take at least minutes):
...
**Example BackendTrafficPolicy for failovers in multiple `backendRefs:`**:
...
spec:
  targetRefs:
    - group: gateway.networking.k8s.io
      kind: HTTPRoute
      name: envoy-ai-gateway-nrp-qwen  # Same name as your AIGatewayRoute
  retry:
    numRetries: 5  # Total retry attempts across all backends
    perRetry:
      backOff:
        baseInterval: 100ms
        maxInterval: 10s
