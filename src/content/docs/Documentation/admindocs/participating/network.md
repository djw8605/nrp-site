...
You can review the [current policy applied to the cluster](https://gitlab.nrp-nautilus.io/prp/calico/-/blob/master/policy_new.yaml). It adds the IPTables rules to the nodes to filter the incoming traffic, additional to multiple other rules that kubernetes uses to route packets between hosts. This is often incompatible with manual iptables rules or firewalld enabled on the node, and because of that we ask to turn those off.
