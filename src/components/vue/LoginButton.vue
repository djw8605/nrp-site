  ...
  <div v-if="user" class="flex items-center justify-center h-full w-full">
    ...
    <Popover ref="op">
      <div class="flex flex-col gap-4 w-[15rem]">
        ...
        <button class="btn-primary !px-4 !py-2 text-sm" @click="handleLogout">Log Out</button>
      </div>
    </Popover>
  </div>
  <div v-else class="flex items-center justify-center h-full w-full">
    <button class="btn-primary !px-4 !py-2 text-sm" @click="handleLogin">Log In</button>
  </div>
</template>
...
const checkLoginStatus = async () => {
  ...
  if (response.ok) {
    ...
    const gravatarUrl =
      'https://www.gravatar.com/avatar/' + CryptoJS.SHA256(data.Email.trim().toLowerCase()) + '?d=robohash&s=65';
